...
exports.action = function(xml, callback, config, SARAH) {
  ...
  var fs = require('fs');
var filePathe = './plugins/courses/courses.json';
var filePathe1 = './plugins/courses/coursesmemoire.json';// json definitif memoire

SCRIBE = SARAH.context.scribe;
  ScribeAskMe = SARAH.ScribeAskMe;
  ScribeSpeak = SARAH.ScribeSpeak;
SCRIBE.activePlugin('courses');
...
datas_xml='<grammar version="1.0" xml:lang="fr-FR" mode="voice" root="rulecourses" xmlns="http://www.w3.org/2001/06/grammar" tag-format="semantics/1.0">\n'
datas_xml+='<rule id="rulecourses" scope="public"><example>Sarah tu es cool</example><tag>out.action=new Object(); </tag>\n'
datas_xml+='<item>Sarah</item>\n'
datas_xml+='<one-of>\n'

for (var i = 0; i < objet1.courses.length; i++) {  
 var cd1=(objet1.courses[i].item);
datas_xml+='<item>ajoute à la liste des courses '+ cd1 +'<tag>out.action.item="' + cd1 +'"</tag></item>\n';   
}

datas_xml+='<item>ajoute à la liste des courses '+ query +'<tag>out.action.item="'+ query +'";</tag></item>\n';
  
for (var i = 0; i < objet1.courses.length; i++) {  
 var cd1=(objet1.courses[i].item);

 datas_xml+='<item>enlève de la liste des courses '+ cd1 +'<tag>out.action.item_out="' + cd1 +'"</tag></item>\n';   
}          

  datas_xml+='<item>enlève de la liste des courses '+ query +'<tag>out.action.item_out="'+ query +'";</tag></item>\n';  
  ...
  datas_xml+='</one-of><tag>out.action._attributes.uri="http://127.0.0.1:8080/sarah/courses";</tag></rule> </grammar>\n'

    var fs = require('fs');
    fs.writeFile("./plugins/courses/courses.xml", datas_xml, function(err) {
      if(err) {console.log(err);}
      else {console.log("plugin courses généré! par cortana: "+query);}
                                                                      }                                                    
                ); 


objet.courses.push({item: query}); new_jsonStr = JSON.stringify(objet);
fs.writeFile(filePathe,new_jsonStr , function (err) {  console.log("valeur rajoutée json courses: " + query);ScribeSpeak ("j'ai rajouté "+query+" a la liste des course") ;});

if (jsonStr1.indexOf(query) > -1  ){ScribeSpeak (query + ", est déja present dans les courses memoires");callback()}

else {
objet1.courses.push({item: query}); new_jsonStr = JSON.stringify(objet1);
fs.writeFile(filePathe1,new_jsonStr , function (err) {  console.log("valeur rajoutée json coursesmemoire: " + query);});
      callback({'tts' : " "})
     }//fin else
     }//fin else
...
ScribeSpeak(objet.courses[i].item+",");SCRIBE.activePlugin('courses');callback();
  }
